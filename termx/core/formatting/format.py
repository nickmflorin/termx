...
class FormatDataClass:
    ...
    def __dict__(self):
        ...
        for fld in fields(self):
            if fld.init:
                if fld.metadata.get('dict_field'):
                    raise RuntimeError(
                        'The metadata field `dict_field` is for non init fields only.')
...
class Format(FormatDataClass, IconFormat, WrapperFormat):
    ...
    def __post_init__(self, icon_before, icon_after, color, background, styles):
        ...
        if self._background and not isinstance(self._background, Color):
            self._background = Color(background)  # Not Currently Used

        if self._background:
            raise NotImplementedError("Background colors currently not supported.")

        self._style = styles or []
        if not isinstance(self._style, Style):
            self._style = ensure_iterable(self._style, coercion=tuple, force_coerce=True)
            self._style = Style(*self._style)
